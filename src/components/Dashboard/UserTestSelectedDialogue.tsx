...
export default function UserTestSelectedDialogue({
  ...
  function shouldYearSelectionShown() {
    ...
    if (selectedCategory === 0 && selectedOption.filter && (selectedOption.subject || selectedOption.exam)) showYearSelection = true
  ...
  function redirectToTestPage() {
    ...
    router.push(`/dashboard/user/test?academy=${selectedOption.academy}&exam=${selectedOption.exam}&subject=${selectedOption.subject}&year=${selectedOption.year}`)
  }
    ...
    <Dialog>
      ...
      <DialogTrigger asChild>
        <Button
          ...
          variant="outline">{title.slice(0, 20)}</Button>
      </DialogTrigger>
      {/* content */}
