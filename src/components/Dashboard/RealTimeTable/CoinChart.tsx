...
function CoinChart() {
  ...
  return (
    <div className=" w-full md:h-auto p-2 rounded-2xl flex flex-col justify-center items-center md:items-center
      ...
      <div className=' w-full h-[30rem] flex flex-col justify-center items-center '>
        {chartTypeOptions.shape === "line" ? (
          <Line
            ...
            data={{
              labels: COIN_DATA.prices.map(data => new Date(data[0]).toLocaleString("default", {
                day: "2-digit",
                month: "short",
              })),
              datasets: [{
      ...
      <div className=' w-full h-auto flex flex-col justify-start items-center space-y-2   '>
        ...
        {Array(6).fill(0).map((_, index) => (
          <div key={index} className=" w-full p-2 text-sm md:text-base border-t-2 border-gray-100 flex justify-between items-center ">
